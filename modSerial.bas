...
'Testing
'Private GPSH As Integer

' Definitions for the RS232 Communication
'
'-------------------------------------------------------------------------------
...
'Recycler Response type
Private Type RSPType
        CODE As Byte
        param As Byte
        EQStatus(1 To 2) As Byte
        BSStatus(1 To 2) As Byte
        ERRCode As Byte
        Data() As Byte
End Type
...
Public udtCommError As COMM_ERROR
...
'   strPort     - COM port name. (COM1, COM2, COM3, COM4, \\.\COM5 ...)
...
Dim udtCommTimeOuts As COMMTIMEOUTS
    ...
    'If Dir("D:\gps.txt") = "" Then
    '    GPSH = 0
    'Else
    '    GPSH = FreeFile
    '    Open "d:\gps.txt" For Binary As #GPSH
    '    Exit Function
    'End If
    ...
    'Disable ALL Hardware Flow Control, and Enable Binary Mode
    udtPorts(intPortID).udtDCB.fBitFields = &H8000
...
'   Error Code  - 0 = No Error.
'-------------------------------------------------------------------------------
Public Function CommRead(intPortID As Integer, bData() As Byte, _
...
Dim lngErrorFlags As Long, udtCommStat As COMSTAT
    ...
    'If GPSH <> 0 Then
    '    Static OneSec As Double
    '    If Timer - OneSec < 1 Then CommRead = 0: Exit Function
    '    OneSec = Timer
    '    Get #GPSH, , bData
    '    CommRead = lngSize
    '    Exit Function
    ...
    If lngRdSize Then
        lngRdStatus = ReadFile(udtPorts(intPortID).lngHandle, bData(0), _
        ...
        ReDim Preserve bData(lngBytesRead - 1)
    End If
...
'   Error Code  - 0 = No Error.
'-------------------------------------------------------------------------------
Public Function CommWrite(intPortID As Integer, bData() As Byte) As Long
...
Dim lngWrSize As Long, lngWrStatus As Long
    ...
    lngSize = UBound(bData) + 1

    ' Output the data.
    lngWrStatus = WriteFile(udtPorts(intPortID).lngHandle, bData(0), lngSize, _
...
Public Function CommGetLine(intPortID As Integer, intLine As Integer, _
   blnState As Boolean) As Boolean
    
Dim lngStatus As Long
Dim lngComStatus As Long
...
Public Function CommBytesQue(intPortID As Integer) As Long
Dim lngStatus As Long
Dim lngErrorFlags As Long, udtCommStat As COMSTAT
   ...
   'Return Bytes in the Que
    CommBytesQue = udtCommStat.cbInQue
'If CommBytesQue > 0 Then Debug.Print "Que: "; CommBytesQue
